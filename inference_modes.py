...
def naive_inference(templates, case_idx, question, funcmodel, temperature, top_p, max_gen_len, return_top=5):
    cur_generation = ""
    logs = []
...
def oracle_1_inference(oracle, templates, case_idx, question, funcmodel, temperature, top_p, max_gen_len, return_top=5):
    cur_generation = ""
    logs = []
    funcmodel.inference_mode = "baseline"
    func_calls = []
    oracle = "<" + oracle + ">"
    prompt = templates[oracle].replace("[QUESTION]", question) + cur_generation
    results = funcmodel.generate([prompt], max_gen_len=max_gen_len, temperature=temperature, top_p=top_p,
                                 stop_token=[13], return_top=return_top)
    if return_top > 0:
        results, token_log = results
        logs.append(token_log)

    cur_generation = results[0].replace(templates[oracle].replace("[QUESTION]", question), "")
...
def oracle_2_inference(oracle, templates, case_idx, question, funcmodel, temperature, top_p, max_gen_len, return_top=5):
    cur_generation = ""
    ...
    try:
        ...
        for op in func_map:
            if cur_generation.endswith(op+"("):
                before_op = op
                cur_generation  = cur_generation.replace(op, oracle) # use oracle to replace the generated result
                if op != oracle:
                    is_work = 1
                    op = oracle
                ...
                break # must at most for loop once
